<?php include("C:\Users\User\Downloads\OSPanel\domains\localhost\school\head.php"); ?>
<?php include("C:\Users\User\Downloads\OSPanel\domains\localhost\school\Top.php"); ?>
<!-- ШКОЛЬНАЯ ЖИЗНЬ -->
    <div class="container">
        <div class="life">
            <h3>Фотогаллерея</h3><br>
            <img data-index="0" class="img_gal" src="http://localhost/school/images/gallery/1.png"/>
            <img data-index="1" class="img_gal" src="http://localhost/school/images/gallery/2.png"/>
            <img data-index="2" class="img_gal" src="http://localhost/school/images/gallery/3.png"/>
            <img data-index="3" class="img_gal" src="http://localhost/school/images/gallery/4.png"/>
            <img data-index="4" class="img_gal" src="http://localhost/school/images/gallery/5.png"/>
            <img data-index="5" class="img_gal" src="http://localhost/school/images/gallery/6.png"/>
            <img data-index="6" class="img_gal" src="http://localhost/school/images/gallery/7.png"/>
            <img data-index="7" class="img_gal" src="http://localhost/school/images/gallery/8.png"/>
            <img data-index="8" class="img_gal" src="http://localhost/school/images/gallery/9.png"/>
        </div>
    </div>
    <script src="http://localhost/school/scripts/script_gal.js"></script>
    <div onclick="this.style.display = 'none';" style="background-color:orange;" class="big_img">
        <img src="http://localhost/school/images/gallery/1.png"/>
    </div>
    <div onclick="this.style.display = 'none';"style="background-color:orange;" class="big_img">
        <img src="http://localhost/school/images/gallery/2.png"/>
    </div>
    <div onclick="this.style.display = 'none';"style="background-color:orange;" class="big_img">
        <img src="http://localhost/school/images/gallery/3.png"/>
    </div>
    <div onclick="this.style.display = 'none';" style="background-color:orange;"class="big_img">
        <img src="http://localhost/school/images/gallery/4.png"/>
    </div>
    <div onclick="this.style.display = 'none';" style="background-color:orange;"class="big_img">
        <img src="http://localhost/school/images/gallery/5.png"/>
    </div>
    <div onclick="this.style.display = 'none';"style="background-color:orange;" class="big_img">
        <img src="http://localhost/school/images/gallery/6.png"/>
    </div>
    <div onclick="this.style.display = 'none';"style="background-color:orange;" class="big_img">
        <img src="http://localhost/school/images/gallery/7.png"/>
    </div>
    <div onclick="this.style.display = 'none';"style="background-color:orange;" class="big_img">
        <img src="http://localhost/school/images/gallery/8.png"/>
    </div>
    <div onclick="this.style.display = 'none';" style="background-color:orange;"class="big_img">
        <img src="http://localhost/school/images/gallery/9.png"/>
    </div> 
<?php include("C:\Users\User\Downloads\OSPanel\domains\localhost\school\bottom.php"); ?>